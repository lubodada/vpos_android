...
public class Print_recharge {
	...
	public void prilay(Context context, String orderId, String monery,
			String PaymentMethod, String cashierName, String clientId,
			String Time, String phonenum) {
		...
		// 弹出框
		new_print(context, orderId, cashiername, monery, PaymentMethod,
				clientId, Time, phonenum);
	}

	@SuppressWarnings("static-access")
	public void new_print(Context context, String orderId, String cashiername,
			String monery, String PaymentMethod, String merchantnum,
			String Time, String phonenum) {
		...
		if (su.getPrefboolean(context, "on_off", false)) {
			...
			try {
				...
				try {
					...
					json2.put("content", "充值号码：" + phonenum);
					json2.put("size", "2");
					...
					json6.put("content", " 充值金额：" + monery + "元");
					json6.put("size", "40");
